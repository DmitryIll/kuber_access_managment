...
Переключаюсь на контекст kubereader:

```
kubectl config use-context kubereader-context
```

![alt text](image-58.png)

Проверяю может ли смотреть поды:

![alt text](image-59.png)

В своем неймспейсе да, в дефолтном нет - то что надо.

Смотрю логи контейнера:

```
kubectl logs nginx-my-project-5f7f547676-jj7p4 -c multitool -n my-project-dev
```
...
В общем все получилось - можно подключиться от пользователя с  нужными провами.


#### Еще пробовал остались другие наработки - запрос подписи сертификата вручную, оставляю для истории но не стал доделывать до конца, т.к. выше уже все по сути сделал
---

Еще пробовал вручную как в статье описано:
...
В файле /roor/kubeconf уже обновленные данные - там появилась jean (видимо т.к. он подключен то мкрок8s его подправил):
...
Потому что лишнее beta. В основном решении уже исправил, но, тут уже не буду доделать, т.к. выше все уже отработал с ролями.
